...
function anc_6310_convert_numbers($data)
{
   return preg_replace('/\D/', '', anc_6310_special_character_remove($data));
}
...
function anc_6310_number_counter_install()
{
   ...
   if (!$counter && !$install_default) {
      ...
      $sql = "INSERT INTO {$counter_table} (title, description, numbers_start, numbers, icons, readmore_text, readmore_url, image) VALUES
         ('Work Completed', '{$description}', '10000', '500', 'fas fa-globe', 'Read More', 'https://www.wpmart.com/accordion-faq-pro', 'https://i.postimg.cc/hP8twJ2y/work-completed.png')
         ";
      $wpdb->query($sql);

      $sql = "INSERT INTO {$counter_table} (title, description, numbers, icons, readmore_text, readmore_url, image) VALUES
         ('Lines Of Code', '{$description}', '1427', 'fas fa-book-open', 'Read More', 'https://www.wpmart.com/accordion-faq-pro', 'https://i.postimg.cc/hP8twJ2y/work-completed.png')
         ";
      $wpdb->query($sql);

      $sql = "INSERT INTO {$counter_table} (title, description, numbers, icons, readmore_text, readmore_url, image) VALUES
         ('Cups Of Coffee', '{$description}', '2100', 'fas fa-university', 'Read More', 'https://www.wpmart.com/accordion-faq-pro', 'https://i.postimg.cc/hP8twJ2y/work-completed.png')
         ";
      $wpdb->query($sql);
   }
}
...
if (!function_exists('anc_6310_check_license')) {
   function anc_6310_check_license($key, $autoUpdate = false) {
      ...
      $db_key = sanitize_text_field(get_option('anc_6310_license_key'));

      if (!$db_key) {
            update_option('anc_6310_license_key', sanitize_text_field($key));
      } elseif ($db_key !== $key) {
            update_option('anc_6310_license_key', sanitize_text_field($key));
      }
      ...
      if (is_wp_error($response)) {
            echo "<p class='anc-6310-error-message'>" . esc_html($response->get_error_message()) . "</p>";
            return;
      }

      $license_data = json_decode(wp_remote_retrieve_body($response));
      if (!isset($license_data->success) || !$license_data->success) {
            $message = '<p class="anc-6310-error-message">An error occurred, please try again.</p>';
            if (isset($license_data->error)) {
                  switch ($license_data->error) {
                        case 'invalid_key':
                              $message = '<p class="anc-6310-error-message">You have entered an invalid license key.</p>';
                              break;
                        case 'site_inactive':
                              $message = '<p class="anc-6310-error-message">Your license is not active for this URL.</p>';
                              break;
                  }
            }
            echo $message;
      ...
      if (is_wp_error($tmp_file)) {
            echo '<p class="anc-6310-error-message">Error downloading the file.</p>';
            return;
      }
      ...
      if ($zip->open($tmp_file) === true) {
            ...
            if (!$autoUpdate) {
                  echo "<p class='anc-6310-success-message'>Congratulations! Your license activated successfully.</p>";
            }
      } else {
            echo '<p class="anc-6310-error-message">There was an error unzipping the file.</p>';
      }
   }
}
